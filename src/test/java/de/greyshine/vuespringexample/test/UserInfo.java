...
public abstract class UserInfo {
	...
	public String loginRest(MockMvc mockMvc, UserService userService) {
		...
		try {
			...
			mockMvc.perform(post("/rest/login").param("login", this.login).param("password", this.password))
	...
	public static UserInfo ADMIN = new UserInfo() {
		{
			...
			rights = new String[] { User.RIGHT_ADMIN };
	...
	public static UserInfo TEST2 = new UserInfo() {
		{
			login = "test2";
			password = "test2pwd";
			rights = new String[] { "right1" };
			email = "test2@" + EMAIL_DOMAIN;
		}
	};
}
